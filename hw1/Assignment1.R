...
################################
#ANSWER: It's impossible to combine all this features into one categorical feature,
#        because some movies can belong to more than 1 category.
...
################################
data.summary <-
  data[, c("length", "budget", "rating", "votes", "year")]
data.summary <-
  do.call(
    data.frame,
    list(
      mean = apply(data.summary, 2, na.rm = TRUE, mean),
      sd = apply(data.summary, 2, na.rm =
                   TRUE, sd),
      variance = apply(data.summary, 2, na.rm =
                         TRUE, var),
      min = apply(data.summary, 2, na.rm =
                    TRUE, min),
      quantile.25 = apply(data.summary, 2, na.rm =
                            TRUE, quantile, prob = 0.25),
      median = apply(data.summary, 2, na.rm =
                       TRUE, median),
      quantile.75 = apply(data.summary, 2, na.rm =
                            TRUE, quantile, prob = 0.75),
      max = apply(data.summary, 2, na.rm =
                    TRUE, max),
      range = apply(data.summary, 2, na.rm =
                      TRUE, max) - apply(data.summary, 2, na.rm = TRUE, min)
    )
  )
t(data.summary)
...
#         The missing values are only at the "budget" and "mpaa" columns so we will remove just them.
apply(data["budget"], 2, function(col)
  sum(is.na(col)) / length(col))
apply(data["mpaa"], 2, function(col)
  sum(is.na(col)) / length(col))
...
#         "votes" feature is a counter, so it is more likely for it to act exponentially, as we see in the plot
#         in the previous section. In order to reduce the exp. affect , we will first perform logarithmic scaling to this column.
data["norm.votes"] <- NA
data$norm.votes = log(data$votes)
data$norm.votes <-
  (data$norm.votes - mean(data$norm.votes)) / sd(data$norm.votes)

qqnorm(data$norm.votes, main = "Normal Q-Q Plot - norm.votes") # We can see that the data now acts less skewed.
qqline(y = data[, "norm.votes"])
...
movies.num <-
  c(
    sum(data$Action == TRUE),
    sum(data$Animation == TRUE),
    sum(data$Comedy == TRUE),
    sum(data$Drama == TRUE),
    sum(data$Documentary == TRUE),
    sum(data$Romance == TRUE),
    sum(data$Short == TRUE)
  )
barplot(
  movies.num,
...
votes.length <-
  aggregate(data$rating,
            by = list(data$votes.desc, data$length.desc),
            FUN = mean)
...
# Density plot of ratings by movie length
plot(density(data[data$length.desc == 'Long', 'rating']),
     xlab = "raiting",
     main = "raiting by length",
     col = 'green')
lines(density(data[data$length.desc == 'Medium', 'rating']),
      xlab = "raiting",
      main = "raiting by length",
      col = 'red')
lines(density(data[data$length.desc == 'Short', 'rating']),
      xlab = "raiting",
      main = "raiting by length",
      col = 'black')
legend(
...
# Density plot of ratings by votes quantity
plot(density(data[data$votes.desc == 'Many_votes', 'rating']),
     xlab = "raiting",
     main = "raiting by votes",
     col = 'green')
lines(density(data[data$votes.desc == 'Few_votes', 'rating']),
      xlab = "raiting",
      main = "raiting by votes",
      col = 'red')
legend(
...
################################
scaled <-
  scale(data[, c('year', 'length', 'rating', 'votes')], center = TRUE, scale =
          TRUE)
library(corrplot)
